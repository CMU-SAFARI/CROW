...
public:
    ...
    bool check_iteratively(typename T::Command cmd, const int* addr, long clk);
...
void DRAM<T>::update_num_subarrays(int num_sa) {
    ...
    if(level == T::Level::Bank) {
        while(children.size() < num_sa) {
            DRAM<T>* child = new DRAM<T>(spec, typename T::Level(int(level) + 1));
...
// the aim is improving the performance
template <typename T>
bool DRAM<T>::check_iteratively(typename T::Command cmd, const int* addr, long clk) {
    int cur_level = int(level);
    int child_id = addr[int(cur_level) + 1];
    auto cur_node = this;
